...
if __name__ == "__main__":
    ...
    f_q = torch.rand([4, 1024, 16, 16]).to(device)
    f_q = F.normalize(f_q, dim=1)
    weights_cls_reshape = torch.rand([4, 1024, 1024]).to(device)

    transformer = MultiHeadAttentionOne(
        1, 1024, 1024, 1024, dropout=0.5
    ).to(device)
    #
    out = transformer(weights_cls_reshape, f_q, f_q)
    print(out.size())
